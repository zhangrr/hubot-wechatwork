...
class WechatWorkAdapter extends Adapter {
    constructor(robot) {
        ...
        this.robot.logger.info(`Yunwei: loadding wechatwork Adapter`);
    }
    ...
    onWebhookVerify(req, res) {
        ...
        this.robot.logger.info(`Yunwei: Get Weixin Test Message: ${ciphertext}`);
        if (ciphertext) {
            const message = WechatWorkCrypto.decryptMessage(ciphertext).message;
            this.robot.logger.info(`Yunwei: Send back ${message}`);
            res.send(message);
            return res.end();
        } else {
            return res.end();
    ...
    onWebhook(req, res) {
        if (req.body.xml && req.body.xml.Encrypt) {
            ...
            if (ciphertext) {
                ...
                this.robot.logger.info(`Yunwei: Get Weixin Message: ${xml}`);
    ...
    onMessageReceived(fromUser, message, messageId) {
        ...
        this.robot.logger.info(`Yunwei: ${messageId}: ${fromUser} => ${message}`);
